...
(function () {
            ...
            if ((token.type.toString() === TOK_TYPE_OPERAND) && (token.subtype.length === 0)) {
                if (isNaN(parseFloat(token.value))) {
                    if ((token.value.toString() === 'TRUE') || (token.value.toString() === 'FALSE' ) ||
                        (token.value.toString() === 'true') || (token.value.toString() === 'false')) {
                        token.subtype = TOK_SUBTYPE_LOGICAL.toString();
                    } else {
   ...
   var fromBase26 = excelFormulaUtilities.fromBase26 = function (number) {
    ...
    // TODO - consider jison parser or evaluate formula on the fly from tokens for more robust implementation
    function evaluateFormula(formula, fields) {
        ...
        var code = excelFormulaUtilities.formula2JavaScript(expression);
